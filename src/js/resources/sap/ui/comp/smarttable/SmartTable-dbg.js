...
sap.ui.define([
	...
	var SmartTable = VBox.extend("sap.ui.comp.smarttable.SmartTable", /** @lends sap.ui.comp.smarttable.SmartTable.prototype */ {
		metadata: {
			...
			properties: {
				...
				/**
				 * Enable data state filtering.
				 * @since 1.81
				 */
				enableDataStateFiltering: {
					type: "boolean",
					group: "Behavior",
					defaultValue: false,
					visibility: "hidden"
	...
	/**
	 * Returns the value set for the <code>enableDataStateFiltering</code> property.
	 * @returns {boolean} <code>enableDataStateFiltering</code> property value.
	 * @private
	 * @sap-restricted Fiori Elements
	 */
	SmartTable.prototype.getEnableDataStateFiltering = function() {
		return this.getProperty("enableDataStateFiltering");
	};

	/**
	 * Sets the value for the <code>enableDataStateFiltering</code> property. By setting this property to true, the <code>SmartTable</code>
	 * control enables filtering of the table record for data state messages.
	 *
	 * <b>Note:</b> This feature is only support when the internal table has the <code>sap.m.plugins.DataStateIndicator</code> dependency.
	 * @param {boolean} bEnable <code>enableDataStateFiltering</code> property value.
	 * @private
	 * @sap-restricted Fiori Elements
	 * @returns {sap.ui.comp.smarttable.SmartTable} control instance
	 */
	SmartTable.prototype.setEnableDataStateFiltering = function(bEnable) {
		if (this.getEnableDataStateFiltering() === bEnable) {
			return this;
		}

		this.setProperty("enableDataStateFiltering", bEnable, true);

		if (!bEnable) {
			this._onApplyMessageFilter(bEnable);
			if (this._oDataStateIndicator) {
				this._oDataStateIndicator.detachEvent("dataStateChange", this._onDataStateChange, this);
				this._oDataStateIndicator.detachEvent("messageLinkPressed", this._onMessageLinkPressed, this);
				this._oDataStateIndicator = null;
			}
		}

		this._bApplyFilter = false;
		this._handleDataStateIndicator();
		return this;
	};

	/**
	 * Handles updates for the DataStateIndicator instance by attaching handlers for dataStateChange & messageLinkPressed events.
	 * @private
	 */
	SmartTable.prototype._handleDataStateIndicator = function() {
		if (!this._oTable || !this.getEnableDataStateFiltering()) {
			return;
		}

		var aInnerTableDependents = this._oTable.getDependents();

		if (!aInnerTableDependents.length) {
			return;
		}

		var oDataStateIndicator = aInnerTableDependents.find(function(oDependent) {
			return oDependent.isA("sap.m.plugins.DataStateIndicator");
		});
		...
		this._oDataStateIndicator = oDataStateIndicator;
		this._oDataStateIndicator.attachEvent("dataStateChange", this._onDataStateChange, this);
		var oResourceBundle = Core.getLibraryResourceBundle("sap.ui.comp");
		this._oDataStateIndicator.setMessageLinkText(oResourceBundle.getText("SMARTTABLE_MESSAGESTRIP_FILTER_ITEMS"));
		this._oDataStateIndicator.setMessageLinkVisible(true);
		this._oDataStateIndicator.attachEvent("messageLinkPressed", this._onMessageLinkPressed, this);
		this._oDataStateIndicator.refresh();
	};
	...
	/**
	 * Handler for the messageLinkPressed event on the DataStateIndicator.
	 * @private
	 */
	SmartTable.prototype._onMessageLinkPressed = function() {
		this._bApplyFilter = !this._bApplyFilter;
		this._onApplyMessageFilter(this._bApplyFilter);
	};

	/**
	 * Creates new instance MessageStrip instance for the p13nFilterPanel.
	 *
	 * @returns {undefined|sap.m.MessageStrip} returns undefined if enableDataStateFiltering is disabled else returns a sap.m.MessageStrip instance
	 * @private
	 */
	SmartTable.prototype._getP13nFilterMessageStrip = function() {
		var oMessageStrip, oResourceBundle;

		if (this.getEnableDataStateFiltering()) {
	...
	/**
	 * Handler for the dataStateChange event of the sap.m.plugin.DataStateIndicator.
	 * This function is called when the data state messages change.
	 * @param {object} oEvent - the event object
	 * @private
	 */
	SmartTable.prototype._onDataStateChange = function(oEvent) {
		var aMessages = oEvent.getParameter("filteredMessages") || [];

		this._sDataStateType = oEvent.getSource()._getCombinedType(aMessages).toUpperCase();
	...
	/**
	 * Calls the relevant APIs for filtering the table by data state messages and also updates the message link text of the DataStateIndicator.
	 * @param {boolean} bApplyFilter indicator to apply or remove the data state filtering
	 * @private
	 */
	SmartTable.prototype._onApplyMessageFilter = function(bApplyFilter) {
		this._filterByDataStateError(bApplyFilter);
		var oResourceBundle = Core.getLibraryResourceBundle("sap.ui.comp");

		if (this._oDataStateIndicator && !bApplyFilter) {
			this._oDataStateIndicator.setMessageLinkText(oResourceBundle.getText("SMARTTABLE_MESSAGESTRIP_FILTER_ITEMS"));
			// it is necessary to check the state when rebindTable is triggered
			this._getRowBinding().checkDataState();
		} else if (this._oDataStateIndicator) {
			this._oDataStateIndicator.setMessageLinkText(oResourceBundle.getText("SMARTTABLE_MESSAGESTRIP_CLEAR_FILTER"));
		}
	};
	...
	SmartTable.prototype._toggleShowHideDetails = function(bValue) {
		...
		var aHiddenImportance = [];

		if (this.bHideDetails) {
			aHiddenImportance = Device.system.phone ? ["Low", "Medium"] : ["Low"];
		}
		this._oTable.setHiddenInPopin(aHiddenImportance);
	...
	SmartTable.prototype._createExportColumnConfiguration = function(mCustomConfig) {
		var aColumns, i, iLen, oColumn, oColumnData, sLabel, sPath, sTemplate, sAdditionalPath, oAdditionalColumn, sSplitLabel, oAdditionalProperty, sHAlign, nWidth, sType, inputFormat, oType, oColumnDataType, aSheetColumns = [];
	...
	SmartTable.prototype._aStaticProperties = [
		"entitySet", "ignoredFields", "initiallyVisibleFields", "ignoreFromPersonalisation", "tableType", "useTablePersonalisation", "enableAutoBinding", "persistencyKey", "smartFilterId", "showDetailsButton"
	];
	...
	SmartTable.prototype._reBindTable = function(mEventParams, bForceRebind) {
		...
		this.fireBeforeRebindTable({
			...
			messageFilterActive: !!this._bApplyFilter
		});
	...
	SmartTable.prototype._createContent = function() {
		...
		if (this.getShowDetailsButton()) {
			this._oTable.setHiddenInPopin(Device.system.phone ? ["Low", "Medium"] : ["Low"]);
		}
	...
	SmartTable.prototype._onColumnPress = function(oEvent) {
		...
		this._oSortButton.setVisible(!!sSortProperty && this._bIsSortPanelEnabled);

		if ((sFilterProperty && this._bIsFilterPanelEnabled) || (sSortProperty && this._bIsSortPanelEnabled)) {
	...
	/**
	 * Enables filter on error messages
	 *
	 * @param {boolean} bFilter when enabled it filters based on error message and set infotoolbar
	 * @private
	 */
	SmartTable.prototype._filterByDataStateError = function(bFilter) {
		if (!bFilter) {
			var bRebindRequired = !!this._oMessageFilter;
			...
			var oBinding = this._getRowBinding();
			if (oBinding && this._oDataStateIndicator) {
				var oDataStateIndicator = this._oDataStateIndicator,
					fnDataStateFilter = oDataStateIndicator.getFilter(),
					fnFilter = fnDataStateFilter ? function(oMessage) {
						return fnDataStateFilter(oMessage, oDataStateIndicator.getControl());
					} : undefined;
				oBinding.requestFilterForMessages(fnFilter).then(function(vFilter) {
					if (!vFilter) {
						return;
					}
					this._oMessageFilter = vFilter;
					this.rebindTable();
					this._createInfoToolbar(true);
				}.bind(this));
			}
		}
	};
	...
	SmartTable.prototype._onPopinChanged = function(oEvent) {
		...
		if (aItems.length && (aHiddenInPopin.length || (bHasPopin && !this.bHideDetails))) {
	...
	SmartTable.prototype._createAnalyticalColumn = function(oField, sId) {
		...
		oColumn = new AnalyticalColumn(sId, {
			...
			template: oField.template,
			groupHeaderFormatter: FormatUtil.getInlineGroupFormatterFunction(oField, true, this._getDateFormatSettings())
		});
		return oColumn;
	};
	...
	SmartTable.prototype._createMobileColumn = function(oField, sId) {
		...
		if (this.getShowDetailsButton() && oField.importance) {
	 ...
	 * @param {boolean} bFilterForDataStateMessages boolean indicator which is true when table is filtered by data state messages
	 * @private
	 */
	SmartTable.prototype._createInfoToolbar = function(bFilterForDataStateMessages) {
		...
		if (bFilterForDataStateMessages) {
			var sInfoToolbarText = oResourceBundle.getText("SMARTTABLE_INFOTOOLBAR_FILTERED_BY", [oResourceBundle.getText("SMARTTABLE_INFOTOOLBAR_MESS_" + this._sDataStateType)]);

			this._updateInfoToolbar(sInfoToolbarText);

			if (this._isMobileTable && !oTable.getInfoToolbar()) {
				oTable.setInfoToolbar(this._oInfoToolbar);
			} else if (!this._isMobileTable) {
				var bInfoToolbarAddedToTableExtension = oTable.getExtension().some(function(oExtension) {
					return oExtension.getId() === this._oInfoToolbar.getId();
				}, this);

				if (!bInfoToolbarAddedToTableExtension) {
					oTable.addExtension(this._oInfoToolbar);
				}
			}
		...
		if (this._isMobileTable && this._oCurrentVariant.group && this._oCurrentVariant.group.groupItems) {
			...
			if (sPath) {
				...
				if (oFieldMetadata) {
					...
					fnGroupFunction = FormatUtil.getInlineGroupFormatterFunction(oFieldMetadata, false, this._getDateFormatSettings());
				}
