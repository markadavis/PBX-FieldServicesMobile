...
// }
	...
	VariantManagement.prototype._cleanUpKeyUser = function() {
	...
	VariantManagement.prototype._handleManageSavePressedForKeyUser = function() {
		var sItemKey, aChanges = [];

		if (this._sDefaultChanges) {
			aChanges.push({
				selectorControl: this,
				changeSpecificData: {
					changeType: "defaultDefaultVariant",
					content: { key: this._sDefaultChanges}
			}});
		}
		...
		if (Object.keys(this._mExecuteOnSelectChanges).length > 0) {
			...
			for (sItemKey in this._mExecuteOnSelectChanges) {
				aChanges.push({
					selectorControl: this,
					changeSpecificData: {
						changeType: "executeOnSelect",
						content: { key: sItemKey, selected: this._mExecuteOnSelectChanges[sItemKey]}
				}});
			}
		}
			...
			for (sItemKey in this._mFavoriteChanges) {
					aChanges.push({
						...
						changeSpecificData: {
							...
							content: { key: sItemKey, visible: this._mFavoriteChanges[sItemKey]}
					}});
			}
		}

		this._fGetDataForKeyUser(aChanges);
		this._cleanUpKeyUser();
	};

	VariantManagement.prototype._handleManageCancelPressedForKeyUser = function() {
...
//		}
	...
	/**
	 * Displays the Manage Views dialog for the key user adaptation.
	 * <br><b>Note:</b> This function must only be used internally during the UI adaptation.
	 * @param {string} sStyleClass Name used to indicate UI adaptation
	 * @param {function} fCallBack Called once the dialog has been closed; returns the change-specific data
	 * @protected
	 */
	VariantManagement.prototype.openManageViewsDialogForKeyUser = function(sStyleClass, fCallBack) {

		var oItem, oItems, iItemNo = 0;
		var oManageItem;
		var oNameCell, oTypeCell, oDefaultCell, oExecuteCell, oDeleteCell;
		var sCurrentDefault;
		...
		var fSetFavoriteState = function(oItem, oControl, bSelected) {
			...
			that._setFavoriteIcon(oControl, bSelected);
		};

		var fSelectFavorites = function(oControlEvent) {
			var bSelected = that._isFavoriteSelected(oControlEvent.oSource), oItem = this.getParent();
			if (oItem) {
				fSetFavoriteState(oItem, oControlEvent.oSource, bSelected);
			}
		};

		var fSelectDefault = function(oControlEvent) {
			var bSelected = (oControlEvent.getParameters().selected === true), oItem = this.getParent();
			if (oItem) {

				if (bSelected) {
					that._sDefaultChanges = oItem.getKey();

					var oFavCtrl = oItem.getCells()[VariantManagement.FAV_COLUMN];
					if (oFavCtrl && oItem.getFavorite && !oItem.getFavorite()) {
						fSetFavoriteState(oItem, oFavCtrl, bSelected);
					}

				} else {
					that._sDefaultChanges = null;
				}
			}
		};
		...
		var fExecuteOnSelect = function(oControlEvent) {
			var bSelected = (oControlEvent.getParameters().selected === true), oItem = this.getParent();
			if (oItem) {
				that._mExecuteOnSelectChanges[oItem.getKey()] = bSelected;
			}
		};
		...
		sCurrentDefault = this.getDefaultVariantKey();
		...
		if (this.getUseFavorites()) {
			...
			if (oItem) {
				...
				oItems.splice(0, 0, oItem); // standard always the first item
			}
		}
		for (var iH = 0; iH < oItems.length; iH++) {
			...
			oTypeCell = new Text(this.oVariantManage.getId() + "-type-" + iItemNo, {
				text: this.oResourceBundle.getText("VARIANT_MANAGEMENT_SHARED"),
				wrapping: false
			});
			...
			oDefaultCell = new RadioButton(this.oVariantManage.getId() + "-def-" + iItemNo, {
				...
				select: fSelectDefault
			});

			if ((sCurrentDefault === oItems[iH].getKey()) || (oItems[iH].getKey() === this.getStandardVariantKey()) && (sCurrentDefault === "")) {
				oDefaultCell.setSelected(true);

//				if (oItems[iH].getFavorite) {
//					fSetFavoriteState(oItems[iH], oManageItem.getCells()[0], true);
//				}
			...
			//delete
			oDeleteCell = new Text();
			//oDeleteCell.setVisible(false); // currently not supported
		...
		//hide executeOnSelect and delete
		var aColumns =  this.oManagementTable.getColumns();
		aColumns[VariantManagement.EXEC_COLUMN].setVisible(false);
		aColumns[aColumns.length - 1].setVisible(false);
