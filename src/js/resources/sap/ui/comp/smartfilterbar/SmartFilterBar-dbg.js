...
sap.ui.define([
	...
	SmartFilterBar.prototype._createFieldInAdvancedArea = function(oFilterGroup, oField) {
		oField.factory = function() {
			...
			if (oField.quickInfo) {
				oFilterGroupItem.setLabelTooltip(oField.quickInfo);
			}
	...
	SmartFilterBar.prototype.fireInitialized = function() {
		if (
			...
			this.getPersistencyKey() &&
			this.getConsiderSelectionVariants() &&
			this._oSmartVariantManagement &&
			this._oSmartVariantManagement.getEnabled()
		) {
			try {
	...
	SmartFilterBar.prototype._prepareSelectionVariants = function() {
		...
		if (aSelectionVariants) {
			...
			oSVM.registerSelectionVariantHandler({
			...
			oSVM.applyDefaultFavorites(aVariantKeys, true);
		}
	};
	...
	SmartFilterBar.prototype._defaultSelectionVariantHandling = function(oSelectionVariant) {
		...
		if (!oSVM.isPageVariant()) {
			...
			oSVM._oStandardVariant = oVariantContent;
			return true;
		}
	};
		...
		if (this.getSuppressValueListsAssociation() && this._oFilterProvider) {
			...
			if (this._oFilterProvider && Array.isArray(this._oFilterProvider.aFilterAnnotations)){
				Promise.all(this._oFilterProvider.aFilterAnnotations).then(function(){
					this.fireEvent("valueListAnnotationLoaded");
				}.bind(this));
			}
		}
	};
	...
	/**
	 * Extended FilterBar function _createVisibleFilters to fire valueListAnnotationLoaded correctly
	 * @private
	 */
	...
	SmartFilterBar.prototype._attachValueListAnnotationLoaded = function(oData, fnFunction, oListener) {
		this.attachEventOnce("valueListAnnotationLoaded", oData, fnFunction, oListener);
	};
	/*
