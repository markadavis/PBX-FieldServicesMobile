...
sap.ui.define([
	...
	DateRangeType.prototype._updateProvider = function(oJson, bSync, bFireFilterChange) {
		...
		if (this.oModel.getData().currentoperation.languageText) {
			...
			if (oData.currentoperation.basicLanguageText.indexOf("{0}") >= 0 && oData.currentoperation.basicLanguageText.indexOf("{1}") >= 0) {
				if (oJson.conditionTypeInfo.data.value1 != null && oJson.conditionTypeInfo.data.value1 !== ""
					&& oJson.conditionTypeInfo.data.value2 != null && oJson.conditionTypeInfo.data.value2 !== "") {
						sFormattedText = this._fillNumberToText(oData.currentoperation.basicLanguageText, oJson.conditionTypeInfo.data.value1, oJson.conditionTypeInfo.data.value2);
					}
			...
			} else if (oData.currentoperation.basicLanguageText.indexOf("{0}") >= 0) {
				if (oJson.conditionTypeInfo.data.value1 !== null && oJson.conditionTypeInfo.data.value1 !== "") {
					if (oJson.conditionTypeInfo.data.value1 === 0 && oData.currentoperation.zeroNumberTextKey) {
						sFormattedText = oData.currentoperation.zeroNumberLanguageText;
					} else if (oJson.conditionTypeInfo.data.value1 === 1 && oData.currentoperation.singulareBasicLanguageText) {
				...
				if (oJson.conditionTypeInfo.data.value1 !== null && oJson.conditionTypeInfo.data.value1 !== "") {
					...
					var sValue;
					if ((typeof v1 === "number" || typeof v1 === "string"  && !isNaN(parseInt(v1))) && oData.currentoperation.valueList) {
	...
	DateRangeType.prototype.getFilterRanges = function() {
		...
		if (oCondition.operation === "LASTDAYS") {
			...
			if (!isNaN(oCondition.value1)) {
				oCondition.value1 = this._getTodayFromToValueFrom(parseInt(oCondition.value1));
			}
			if (!isNaN(oCondition.value2)) {
				oCondition.value2 = this._getTodayFromToValueTo(parseInt(oCondition.value2));
			}
		} else {
	...
	DateRangeType.prototype._getTodayFromToValueFrom = function (iValueFrom) {
		if (iValueFrom < 0) {
			var oValueFrom = new UniversalDateUtils.getRange((-1 * iValueFrom), "DAY")[1];
			oValueFrom.setHours(0,0,0);

			return oValueFrom;
		}

		return new UniversalDateUtils.getRange(-iValueFrom, "DAY")[0];
	};

	DateRangeType.prototype._getTodayFromToValueTo = function (iValueTo) {
		if (iValueTo < 0){
			var oValueTo = new UniversalDateUtils.getRange(iValueTo, "DAY")[0];
			oValueTo.setHours(23,59,59);
			return oValueTo;
		}
		return  new UniversalDateUtils.getRange(iValueTo, "DAY")[1];
	};
