...
sap.ui.define([
	...
	VariantConverterFrom.prototype._addRangesAccordingMetaData = function(oContent, oFilterBar, oFilterMetaData, aRanges, oControl, sName) {
		...
		var fgetYearMonthDatePattern = function(aDateParts){
			var rx = new RegExp("[0-9]{8}");
			if (aDateParts[0].match(rx)){
				var aSplitedResultDate = [];
				var sYear = aDateParts[0].substring(0,4);
				var sMonth = aDateParts[0].substring(4,6);
				var sDay = aDateParts[0].substring(6,9);
				aSplitedResultDate[0] = sYear;
				aSplitedResultDate[1] = sMonth;
				aSplitedResultDate[2] = sDay;
				return aSplitedResultDate;
			} else {
				return aDateParts;
			}
		};

		var fgetTimeZoneFormat = function(sMillis){
			//Remove timezone.
			if (sMillis.indexOf(" ") !== -1) {
				sMillis = sMillis.split(" ")[0];
			} else if (sMillis.indexOf("-") !== -1){
				sMillis = sMillis.split("-")[0];
			} else if (sMillis.indexOf('+') !== -1){
				sMillis = sMillis.split('+')[0];
			}
		...
		var fcreateDateObject = function(sDateString/* "2015-02-28T23:00:00.000" || "20160708" || "2015-02-28T23:00:00.000 01"*/) {
			var sDateTimeString = sDateString.split(".")[0];
			var sMillis = sDateString.split(".")[1];
			if (sMillis){
				sMillis = fgetTimeZoneFormat(sMillis);
			}
			var aParts = sDateTimeString.split("T");
			var aDateParts = aParts[0].split("-");
			if (aDateParts.length === 1) {
				aDateParts = fgetYearMonthDatePattern(aDateParts);
			}
			if (aParts.length < 2) { // There are no Time parts if aParts length is less than 2
				return sMillis ? new Date(aDateParts[0], aDateParts[1] - 1, aDateParts[2], sMillis) : new Date(aDateParts[0], aDateParts[1] - 1, aDateParts[2]);
			} else {
			...
			if (sValue && oFilterMetaData) {
				...
				if (oFilterMetaData.filterType === "numeric") {
					...
					switch (oFilterMetaData.type) {
						...
						break;
						case "Edm.String":
							if (oFilterMetaData.isCalendarDate){
								oValue = fgetDateFormatLogic(oFilterMetaData, oFilterBar, sValue, oValue);
							}
						break;
						...
						case "Edm.Date":
							...
							if (sTime){
								sTime = fgetTimeZoneFormat(sTime);
							}
