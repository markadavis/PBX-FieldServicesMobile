...
sap.ui.define([
	'sap/ui/core/library', 'sap/m/VBox', 'sap/m/Bar', 'sap/m/Button', 'sap/m/CheckBox', 'sap/m/Label', 'sap/m/Link', 'sap/m/List', 'sap/m/Panel', 'sap/m/SearchField', 'sap/m/Text', 'sap/m/Toolbar', 'sap/m/ToolbarSpacer', 'sap/ui/Device', 'sap/ui/comp/state/UIState', './VariantConverterFrom', './VariantConverterTo', 'sap/ui/comp/library', 'sap/ui/comp/smartvariants/PersonalizableInfo', 'sap/ui/comp/smartvariants/SmartVariantManagementUi2', 'sap/ui/comp/variants/VariantManagement', 'sap/ui/core/InvisibleText', 'sap/ui/core/Icon', 'sap/ui/core/Title', 'sap/ui/layout/Grid', 'sap/ui/layout/GridRenderer', 'sap/ui/layout/GridData', 'sap/ui/layout/HorizontalLayout', 'sap/ui/layout/VerticalLayout', 'sap/ui/layout/AlignedFlowLayout', 'sap/ui/comp/util/IdentifierUtil', 'sap/m/library', 'sap/ui/model/json/JSONModel', 'sap/ui/comp/filterbar/FilterGroupItem', 'sap/base/Log', 'sap/base/util/merge', 	'sap/ui/base/ManagedObject', 'sap/ui/core/ShortcutHintsMixin'
], function(coreLib, VBox, Bar, Button, CheckBox, Label, Link, List, Panel, SearchField, Text, Toolbar, ToolbarSpacer, Device, UIState, VariantConverterFrom, VariantConverterTo, library, PersonalizableInfo, SmartVariantManagementUi2, VariantManagement, InvisibleText, Icon, Title, Grid, GridRenderer, GridData, HorizontalLayout, VerticalLayout, AlignedFlowLayout, IdentifierUtil, mLibrary, JSONModel, FilterGroupItem, Log, merge, ManagedObject, ShortcutHintsMixin) {
	"use strict";
	...
	var FilterBar = Grid.extend("sap.ui.comp.filterbar.FilterBar", /** @lends sap.ui.comp.filterbar.FilterBar.prototype */
	{
		metadata: {
			...
			properties: {
				...
				/**
				 * Sets whether the filter bar should look like the filters area in a ValueHlepDialog.
	...
	FilterBar.prototype.init = function() {
		...
		*/
		this._nMaxFiltersByDefault = 9999;
	...
	FilterBar.prototype.onBeforeRendering = function () {
		...
		if (this.getIsRunningInValueHelpDialog()) {
			...
			if (this._oBasicSearchField && this._oBasicSearchField.isA("sap.m.SearchField")) {
				this._oBasicSearchField.setWidth("50%");
				this._oBasicSearchField.setShowMagnifier(true);
				this._oBasicSearchField.setShowSearchButton(true);
			}
		...
		if (this._oHideShowButton) {
			...
			if (this._oHideShowButton.getVisible()) {
				if (this.getFilterBarExpanded()) {
					...
					this._oHideShowButton.setTooltip(this._oRb.getText("FILTER_BAR_VH_HIDE_FILTERS"));
				} else {
					this._oHideShowButton.setText(this._oRb.getText("FILTER_BAR_VH_SHOW_FILTERS"));
					this._oHideShowButton.setTooltip(this._oRb.getText("FILTER_BAR_VH_SHOW_FILTERS"));
				}
			}
		}
			...
			if (sPropertyName === "visibleInFilterBar" || sPropertyName === "visible") {
				...
				if (oItem) {
					if (sPropertyName === "visibleInFilterBar") {
						...
						var bPartOfBasicGroup = this._isFilterItemInBasicGroup(oItem.filterItem);

						if (!bVisibleInFilterBar && !bPartOfBasicGroup) {
							oEvent.oSource.setPartOfCurrentVariant(false);
						}

						if (!this.getAdvancedMode() && !this._oDialog && !this._oAdaptFiltersDialog) {
							if (this._bOldAdaptFilterDialog && !bPartOfBasicGroup || !this._bOldAdaptFilterDialog) {
...
// }
	...
	/**
	 * Returns promise which will be resolved when the initialized event is fired.
	 * @returns {Promise}
	 */
	...
	FilterBar.prototype._getFilters = function() {
		...
		if (this._aFields && this._aFields.length > 0) {
			...
			for (i = 0; i < this._aFields.length; i++) {
				...
				if (oField.visibleInAdvancedArea || (oField.groupName === FilterBar.INTERNAL_GROUP) || aFiltersWithValues.indexOf(oField.name) !== -1) {
...
// };
				...
				if (n) {
					...
					if (oGroup && oGroup.items) {
						for (i = 0; i < oGroup.items.length; i++) {
							...
							if (oItem && oItem.filterItem) {
								this._setPersVisibility(aPersData, oItem.filterItem, aFiltersNotPartOfCurrentVariant);
							}
						}
	 ...
	 */
	FilterBar.prototype._setPersVisibility = function(aPersData, oFilterItem, aFiltersNotPartOfCurrentVariant) {
...
//			}
	...
	FilterBar.prototype._selectionChangedInFilterDialog = function(oCheckBox, oFilterItem) {
		...
		this._selectionChangedInFilterDialogByValue(oCheckBox.getSelected(), oFilterItem);
	};
	...
	FilterBar.prototype._selectionChangedInFilterDialogByValue = function(bValue, oFilterItem) {
		...
		if (this._mAdvancedAreaFilter && this._bOldAdaptFilterDialog) {
			for (var n in this._mAdvancedAreaFilter) {
				if (n) {
					var oGroupElement = this._mAdvancedAreaFilter[n];
					if (oGroupElement && oGroupElement.items) {
						for (var i = 0; i < oGroupElement.items.length; i++) {
							var oItem = oGroupElement.items[i];
							if (oFilterItem === oItem.filterItem) {
								if (oItem.hasOwnProperty("selectedAsPoCV")) {
									oItem.selectedAsPoCV = bValue;
								}
								break;
							}
						}
	...
	FilterBar.prototype._determineNotAssignedFiltersCount = function(sGroupName) {
		...
		if (this._mAdvancedAreaFilter[sGroupName] && this._mAdvancedAreaFilter[sGroupName].items) {
			for (i = 0; i < this._mAdvancedAreaFilter[sGroupName].items.length; i++) {
				...
				if (!this._checkIfFilterHasValue(oFilterItem.getName()) && !oFilterItem.getVisibleInFilterBar()) {
					nCount++;
				}
			}
		}
	...
	FilterBar.prototype._addFieldToFiltersDialog = function(oItem, oFormContainer, aFilters) {
		...
		var bShowFilter = this._isFilterItemInBasicGroup(oItem.filterItem) || this._determineVisibility(oItem.filterItem);
...
// }));
	...
	FilterBar.prototype._createHeaderAddToFilterBar = function(oFormContainer) {
		...
		if (this._isNewFilterBarDesign() || !this._isPhone()) { // label 'Add To Filterbar'
			...
			var oTitle = new MTitle({
				text: ManagedObject.escapeSettingsValue(oFormContainer.getTitle().getText()),
		...
		for (n in this._mAdvancedAreaFilter) {
			if (n && this._mAdvancedAreaFilter[n].items) {
				...
				oGroupTitle = new Title({
					text: ManagedObject.escapeSettingsValue(sGroupName)
				});

				oFormContainer = new FormContainer({
					title: ManagedObject.escapeSettingsValue(oGroupTitle)
				});
	 ...
	 * @private
	 */
	FilterBar.prototype._searchRequested = function(oFilterDialog) {
		...
		if (bInErrorState) {
			...
			if (this._oFiltersSearchField && this._oFiltersSearchField.getValue()) {
	 ...
	 * @private
	 */
	FilterBar.prototype._closeDialogAndRestoreFilterBar = function() {

		this._sSearchCriteriaInFiltersDialog = null;

		this._oVariant = {};

		if (this._oFilterDialogForm) {
			this._oFilterDialogForm.removeAllFormContainers();
		}
	...
	FilterBar.prototype.setContentWidth = function(nWidth) {
		...
		if (this._oAdaptFiltersDialog) {
			this._oAdaptFiltersDialog.setContentWidth(nWidth + "px");
		}
	};

	/**
	 * Enables to add application specific content as a customer view to the new adapt filters dialog.
	 * @protected
	 * @param {sap.m.SegmentedButtonItem} oCustomSegmentedButtonItem the custom button
	 * @param {function} fCustomLiveSearch callback triggered by search
	 * @returns {sap.ui.core.Control} oCustomViewContent displayed on the customer view.
	 */
	FilterBar.prototype.addAdaptFilterDialogCustomContent = function(oCustomSegmentedButtonItem, fCustomLiveSearch, oCustomViewContent) {
		this._mCustomViewInfo = {
			item: oCustomSegmentedButtonItem,
			search : fCustomLiveSearch,
			content : oCustomViewContent
		};
	};
	...
	FilterBar.prototype.setContentHeight = function(nHeight) {
		...
		if (this._oAdaptFiltersDialog) {
			this._oAdaptFiltersDialog.setContentHeight(nHeight + "px");
		}
	};

	FilterBar.prototype._createStructureForAdaptFiltersDialog = function() {
		var oGroup, oItem, mState = { filtersOnFilterBar: {items: []}, allFilters: []};
				...
				if (n) {
					...
					if (oGroup.items) {
						for (var i = 0; i < oGroup.items.length; i++) {
							...
							if (oItem.filterItem.getVisibleInFilterBar()) {
								mState.filtersOnFilterBar.items.push({ name: oItem.filterItem.getName()});
							}
	...
	FilterBar.prototype._restoreControls = function(fVisibilityChanged) {
				...
				if (n) {
					...
					if (oGroup.items) {
						for (var i = 0; i < oGroup.items.length; i++) {
							...
							if (oItem.checkbox) {
								delete oItem.checkbox;
							}
							if (oItem.filterItem && !oItem.filterItem.getHiddenFilter() && oItem.control && oItem.container) {

								if (oItem.fnDestroy) {
									oItem.control.destroy = oItem.fnDestroy;
									delete oItem.fnDestroy;

									aContent = oItem.container.getContent();
									oItem.container.removeContent(aContent[1]);
									aContent[1].destroy();
	...
	FilterBar.prototype._expandGroup = function(oItem)	{
		if (this._oAdaptFiltersDialog && this._oAdaptFiltersDialogModel && this._oGroupPanel && oItem && oItem.filterItem) {
				...
				if (aItemsGrouped[i].group === sGroupName) {
					for (var j = 0; j < aItemsGrouped[i].items.length; j++) {
						if (aItemsGrouped[i].items[j].name === oItem.filterItem.getName()) {
							this._oGroupPanel.setGroupExpanded(aItemsGrouped[i].items[j].group, true);
							return;
						}
					}
					return;
	...
	var P13nBuilder, GroupPanelBase;

	FilterBar.prototype._getEntitiesLazy = function() {
		if (P13nBuilder && GroupPanelBase & CustomListItem) {
		...
		return sap.ui.getCore().loadLibrary('sap.ui.mdc', {
			...
			return new Promise(function(resolve) {
				...
				], function(fnP13nBuilder, fnGroupPanelBase, fnCustomListItem) {
					P13nBuilder = fnP13nBuilder;
					GroupPanelBase = fnGroupPanelBase;
...
//			}
	...
	FilterBar.prototype._showAdaptFilterDialog = function()	{
		...
		return new Promise(function(fResolve) {
			...
			this._getEntitiesLazy().then(function() {
				...
				if (!this._oAdaptFiltersDialog) {
								...
								for (var i = 0; i < aItems.length; i++) {
									if (aItems[i].hasOwnProperty("filterItem") && aItems[i].filterItem === oEvent.oSource) {
										if (sPropertyName === "visible") {
											...
											aItems[i].selected = oEvent.oSource.getVisibleInFilterBar();
					...
					var oP13nData = P13nBuilder.prepareP13nData(mState.filtersOnFilterBar, mState.allFilters);
					...
					var fChange = function(oEvent) {
						var bAdd = oEvent.getParameter("reason") === "Add";

						var mItem = oEvent.getParameter("item");
						if (!mItem) {
							return;
						}

						if (mItem.name === "_BASIC_SEARCH_FIELD") {
							mItem.selected = true;
							return;
						}

						var oItem = this._determineItemByName(mItem.name, mItem.group);
						if (oItem && oItem.filterItem) {
							if (oItem.filterItem.getMandatory() && !bAdd && !this._hasFilterValue(oItem.filterItem)) {
								mItem.selected = true;
							} else {
								this._selectionChangedInFilterDialogByValue(mItem.selected, oItem.filterItem);
							}
						}
					}.bind(this);


					var oFilterPanel = new GroupPanelBase({
						change: fChange,
					...
					this._oGroupPanel = oFilterPanel;
					var sModelName = "$p13n";

					oFilterPanel.setItemFactory(function(sId, oContext) {

						var oFilterItem, oControl, oItem, oObj;
						...
						oFilterItem = oItem.filterItem;

						oControl = oItem.control;
						if (!oControl) {
							return undefined;
						}
						var sItemId = IdentifierUtil.replace(oObj.group + "-" + oFilterItem.getName() + "-selectFiltersItem");

						if (oItem.container && oItem.container.getContent()[1] === oControl) {
							oItem.container.removeContent(oItem.control);
							oItem.container.insertContent(oItem.control.clone(), 1);

							//TODO: needs to be discussed
							//whenever filtering occurs on the list, the controls will be destroyed.
							//On the other hand:
							//-custom controls could be not bound, so the original control has to be added and
							//-filters's are marked in error-state, only on original controls
							if (oItem.control.destroy) {
								oItem.fnDestroy = oItem.control.destroy;
								oItem.control.destroy = function() {};
							}
						}

						var oLabel = new Label({
							text: "{" + sModelName + ">label}"
						});
						//oControl.addAriaLabelledBy(oLabel);

						oLabel.setLabelFor(oControl);
						if (oFilterItem && oFilterItem.getMandatory()) {
//							oControl.setRequired(true);
							oLabel.setRequired(true);
						}

						var oListItem = new CustomListItem(sItemId, {
							selected:"{" + sModelName + ">selected}",
							visible: "{" + sModelName + ">visibleInDialog}",
							content: [ oLabel, oControl]
						});

//						oItem.checkbox = oListItem.getModeControl();
//
//						this._adaptMandatoryForFilter(oFilterItem);
						return oListItem;
					}.bind(this));
					...
					if (this._mCustomViewInfo && this._mCustomViewInfo.item && this._mCustomViewInfo.item.getKey()) {
						...
						var bMandatoryNotSet = false;
						if (this._validateMandatoryFields) {
							bMandatoryNotSet = !this._validateMandatoryFields();
						}

						oFilterPanel.switchViewMode(bMandatoryNotSet ? oFilterPanel.GROUP_KEY : this._mCustomViewInfo.item.getKey());
					}
...
//						this.fireFiltersDialogBeforeOpen({newDialog: true});
						...
						this._oAdaptFiltersDialog.attachBeforeClose(function() {
							this._restoreControls(fVisibilityChanged);
						...
						this._oAdaptFiltersDialog.attachAfterClose(function() {
							...
							if (this._bOKFiltersDialogTriggered && this._bDirtyViaDialog) {
								if (!this.isLiveMode || !this.isLiveMode()) {
	...
	FilterBar.prototype._showFilterDialog = function() {
		...
		if (!this._oFilterDialog) {
			...
			this._oFilterDialog = new Dialog(this.getId() + "-adapt-filters-dialog", {
				...
				draggable: true,
				contentWidth: "60rem",
				title: this._oRb.getText(
					this._isNewFilterBarDesign() ? "FILTER_BAR_ADAPT_FILTERS_DIALOG" : "FILTER_BAR_ADV_FILTERS_DIALOG"
				),
				verticalScrolling: true,
				initialFocus: this._oFiltersSearchField,
				subHeader: new Bar({contentMiddle: this._oFiltersSearchField}),
				beforeOpen: function() {
						this.fireFiltersDialogBeforeOpen();
					}.bind(this),
				beforeClose: function() {
						if (!this._bOKFiltersDialogTriggered) {
							this._cancelFilterDialog(this._bVariantSaveFiltersDialogTriggered);
						}

						delete this._oFilterDialog.bDoNotAddToBasicArea;
						this._closeDialogAndRestoreFilterBar();
					}.bind(this),
				afterClose: function() {
						this._oBasicAreaLayout.rerender(); // seems to be required from 1.32...
			...
			}).addStyleClass("sapUiPopupWithPadding sapUiCompFilterBarDialog sapMH4FontSize sapMH4Style");
		...
		this._oFilterDialogForm = this._createFiltersAndAdaptBasicArea();
		if (!this._oFilterDialogForm) {
			Log.error("Content for Advanced Filters Dialog could not be created");
			return;
		}
		this._oFilterDialog.addContent(this._oFilterDialogForm);

		this._bOKFiltersDialogTriggered = false;
		this._bVariantSaveFiltersDialogTriggered = false;
		this._oFilterDialog.bDoNotAddToBasicArea = true;

		this._addFilterDialogButtons(this._oFilterDialogForm);
	...
	FilterBar.prototype.isDialogOpen = function() {
		return (this._oFilterDialog || this._oAdaptFiltersDialog ) ? true : false;
	};
	...
	FilterBar.prototype._addFilterDialogButtons = function(oForm) {
		...
		oSearchButton = new Button(this.getId() + "-btnGoFilterDialog", {
			...
			press: function() {
				oSearchButton.focus();
				that._dialogSearch(that._oFilterDialog);
			},
		...
		if (!this.getAdvancedMode()) {
			...
			oCancelButton = new Button(this.getId() + "-btnCancelFilterDialog", {
				...
				press: function() {
					that._dialogCancel(that._oFilterDialog);
				},
				layoutData: new OverflowToolbarLayoutData({
	...
	FilterBar.prototype._adaptMandatoryForVisibleFilters = function() {
		var oFilterItem, aVisibleAdvancedItems = this._retrieveVisibleAdvancedItems();

		for (var i = 0; i < aVisibleAdvancedItems.length; i++) {
			oFilterItem = aVisibleAdvancedItems[i].filterItem;
			this._adaptMandatoryForFilter(oFilterItem);
		}
	};

	FilterBar.prototype._dialogSearch = function(oAdaptFiltersDialog) {

		this.fireFiltersDialogSearch();

		this._searchRequested(oAdaptFiltersDialog);
	};
	...
	FilterBar.prototype._dialogRestore = function() {
		...
		this._adaptMandatoryForVisibleFilters();
	...
	FilterBar.prototype.initShowAllFiltersButton = function() {
		this._oShowAllFiltersButton = new Button({
	...
	FilterBar.prototype._addControlToBasicAreaContainer = function(oFilterItem, oControl, oLabel) {
		...
		if ((!this.getAdvancedMode() || this.getIsRunningInValueHelpDialog()) && (!this._oFilterDialog || (!this._oFilterDialog.hasOwnProperty("bDoNotAddToBasicArea")))) {
...
// }
			...
			if (aAdvacedElements && aAdvacedElements.items) {
				...
				for (i = 0; i < aAdvacedElements.items.length; i++) {
					...
					if (oItem && oItem.filterItem) {
						...
						oItem.initialPartOfCurrentVariant = this._checkIfFilterHasValue(oItem.filterItem.getName()); //oItem.filterItem.getPartOfCurrentVariant();
						...
						oLabel.setText(oItem.filterItem.getLabel());
						oLabel.setTooltip(oItem.filterItem.getControlTooltip() || oItem.filterItem.getLabelTooltip());

						oListItem.addContent(oLabel);

						oList.addItem(oListItem);
						if (oItem.filterItem.getVisibleInFilterBar() || oItem.selectedAsPoCV || this._checkIfFilterHasValue(oItem.filterItem.getName())) {
							oList.setSelectedItem(oListItem);
						}

						if (oItem.filterItem.getMandatory() && oItem.checkbox && !oItem.checkbox.getEnabled()) {
							oListItem.getModeControl(true).setEnabled(false);
						}
					}
				}
			}
	 ...
	 * @returns {boolean} Returns true if the filter item will be visible, false otherwise
	 */
	FilterBar.prototype._rerenderItem = function(oItem, bHideExplicitly) {
		...
		if (oItem) {
			...
			if (oItem.formelement) {
				oItem.formelement.setVisible(oItem.filterItem.getVisible() &&
					(oItem.filterItem.getVisibleInFilterBar() || this._isFilterItemInBasicGroup(oItem.filterItem) || this._checkIfFilterHasValue(oItem.filterItem.getName())));
				if (oItem.checkbox) {
	...
	FilterBar.prototype._rerenderFilters = function() {
		...
		var nTotalVisibleFilterItems = arrVisibilityMap.filter(function(elem){return elem.visible;}).length;
		var nMaxItems = nTotalVisibleFilterItems > this._nMaxFiltersByDefault ?
	...
	/**
	 * Function that defines the sort order for _aListItems.sort()
	 * used only for _createAddRemoveFiltersDialog method
	 * @private
	 */
	FilterBar.prototype._sortListItems = function(oFirstElement, oSecondElement) {
		var sFirstLabel, sSecondLabel,
			oFirstContent = oFirstElement.data('item'),
			oSecondContent = oSecondElement.data('item');

		if (oFirstContent && oFirstContent.filterItem && oSecondContent && oSecondContent.filterItem) {
			sFirstLabel = oFirstContent.filterItem.getLabel();
			sSecondLabel = oSecondContent.filterItem.getLabel();
		}

		return sFirstLabel.localeCompare(sSecondLabel);
	};
	...
	FilterBar.prototype._createAddRemoveFiltersDialog = function(sGroupName, oLink) {
		...
		// Sorting the list items alphabetically
		if (this._aListItems && this._aListItems.length > 1) {
			this._aListItems.sort(this._sortListItems);
		}
	...
	FilterBar.prototype._applyVariant = function(oVariant, sContext, bInitial, sVersion) {
		...
		if (oVariant) {
			...
			var aFilterNonPartOfCurrentVariant = this._reapplyVisibility(aPersFields);
				...
				if (n) {
					...
					if (oGroupElement && oGroupElement.items) {
						...
						for (i = 0; i < oGroupElement.items.length; i++) {
							...
							if (oGroupElement.items[i] && oGroupElement.items[i].selectedAsPoCV) {
								delete oGroupElement.items[i].selectedAsPoCV;
							}
						}
					}
				}
	 ...
	 * @returns True if <code>oFilterItem</code> is part of <code>FilterBar.INTERNAL_GROUP</code>
	 * @private
	 */
	...
	FilterBar.prototype.exit = function() {
		...
		if (this._oFilterDialogForm) {
			this._oFilterDialogForm.destroy();
			this._oFilterDialogForm = null;
		}
	...
	// Deffered will be added in 1.90 as base/util module. Until then we will use it from here
