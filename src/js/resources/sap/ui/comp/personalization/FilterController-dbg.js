...
sap.ui.define([
	...
	FilterController.prototype.getPanel = function(oPayload) {
		...
		return new Promise(function(resolve) {
			...
			sap.ui.require([
				...
				var fnSuggestCallback = function(oControl, sFieldName, bIsIncludePanel) {
					...
					if (oSmartFilter && oSmartFilter._oFilterProvider && oSmartFilter._oFilterProvider._aFilterBarMultiValueFieldMetadata) {
						...
						if (oCurrentFieldMetadata) {
							if (bIsIncludePanel === true) {
								oPanel._oIncludeFilterPanel.setDisplayFormat(oCurrentFieldMetadata.displayFormat);
							} else if (bIsIncludePanel === false) {
								oPanel._oExcludeFilterPanel.setDisplayFormat(oCurrentFieldMetadata.displayFormat);
							}
						}
					}
				...
				oPanel._oIncludeFilterPanel._fSuggestCallback = function(oControl, sFieldName) {
					return fnSuggestCallback(oControl, sFieldName, true);
				};

				oPanel._oExcludeFilterPanel._fSuggestCallback = function(oControl, sFieldName) {
					return fnSuggestCallback(oControl, sFieldName, false);
				};
	...
	*/
	FilterController.prototype._getSmartFilterBar = function() {
		...
		if (!oSmartFilter && oTable && this.getTableType && this.getTableType() === CompLibrary.personalization.TableType.ChartWrapper){
