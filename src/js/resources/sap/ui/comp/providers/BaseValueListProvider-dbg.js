...
sap.ui.define([
	...
	BaseValueListProvider.prototype._isFieldHiddenInFilterBar = function (oFilterBar, sFieldName) {
		var oFilterItem = oFilterBar._getFilterItemByName(sFieldName) || oFilterBar._getFilterItemByName(ANALYTICAL_PARAMETER_PREFIX + sFieldName);
				...
				if (sLocalFieldName) {
					...
					if (sValueListFieldName !== this.sKey) {
						...
						while (i--) {
							...
							if (this.sContext === "SmartFilterBar" && this._isFieldHiddenInFilterBar(this.oFilterProvider._oSmartFilter, sLocalFieldName)) {
								continue;
							}
		...
		for (i = 0; i < aDataModelRow.length; i++) {
			...
			if (sKey || (sKey === "")) {
				...
				for (j = 0; j < aCurrentTokens.length; j++) { // remove tokens, which will be re-created with a text
					...
					if (sCurrentKey && sCurrentKey === sKey) {
							break;
