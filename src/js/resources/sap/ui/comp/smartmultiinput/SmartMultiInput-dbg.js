...
sap.ui.define([
	...
	SmartMultiInput.prototype._generateDisplayText = function (sText) {
		...
		var oControlDomRef = this.getDomRef(),
			...
			iNMoreWidth = _getPixelWidth("999 " + oBundle.getText("POPOVER_DEFINE_MORE_LINKS")), // more link is assumed to have 3 digit maximum tokens
			oText = new Text({
				wrapping: false
			}),
			oFirstVisibleText,
		...
		oText.setText(sText);
		var iTextWidth = _getPixelWidth(sText);
		...
		if (this._iHiddenLabelsCount === 0 && this._iHBoxWidth <= iMultiInputWidth) {
			...
			while (this._iHBoxWidth && this._iHBoxWidth > iMultiInputWidth - iNMoreWidth) {
				...
				if (this._oHBox.getItems()[0]) {
					var iFirstVisibleTextLength = _getPixelWidth(this._oHBox.getItems()[0].getText());
					this._iHBoxWidth -= iFirstVisibleTextLength;
	...
	SmartMultiInput.prototype.exit = function () {
		...
		//unbind all aggregation
		if (this.getBindingContext()) {
			if (this._oMultiInput) {
				this._oMultiInput.unbindAggregation("tokens");
			}
			if (this._oMultiComboBox) {
				this._oMultiComboBox.unbindItems();
			}
			if (this._oTokenizer) {
				this._oTokenizer.unbindAggregation("tokens");
			}
		}
