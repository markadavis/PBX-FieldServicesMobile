	...
	var SmartVariantManagement = VariantManagement.extend("sap.ui.comp.smartvariants.SmartVariantManagement", /** @lends sap.ui.comp.smartvariants.SmartVariantManagement.prototype */
	{
		metadata: {
			...
			properties: {
				...
				/**
				 * Collects changes from a lower layer than the end user layer for SAPUI5 flexibility.
				 * <b>Note</b>: This property must not be bound.<br>
				 * <b>Note</b>: This property is used exclusively for SAPUI5 flexibility. Do not use it otherwise.
				 *
				 * @param {map} adaptationInfo Object containing all the information related to changes
				 * @param {string} adaptationInfo.defaultVariant Key of the default variant
				 * @param {map} adaptationInfo.addFavorites Map
				 * @param {string} adaptationInfo.addFavorites.key Variant key
				 * @param {boolean} adaptationInfo.addFavorites.visible Defines the state of the favorite
				 * @param {map} adaptationInfo.executeOnSelect Map
				 * @param {string} adaptationInfo.executeOnSelect.key Variant key
				 * @param {boolean} adaptationInfo.executeOnSelect.selected Defines the state of Apply Automatically
				 *
				 * @since 1.81
				 */
				adaptationInfo: {
...
// }
		...
		// favorites handling
		this._applyAdaptionInfoFavorites();
	...
	SmartVariantManagement.prototype._applyAdaptionInfoFavorites = function() {
		var oAdaptionInfo = this.getProperty("adaptationInfo");
		if (oAdaptionInfo && oAdaptionInfo.addFavorites) {

			// TODO: clarify: key user: default variant, but favorite is not set/false
			//var sDefaultKey = this._getDefaultVariantKey();

			for (var sKey in oAdaptionInfo.addFavorites) {
				if (sKey === this.STANDARDVARIANTKEY) {
					this.setStandardFavorite(oAdaptionInfo.addFavorites[sKey]);
				} else {
					var oItem = this.getItemByKey(sKey);
					if (oItem) {
						oItem.setFavorite(oAdaptionInfo.addFavorites[sKey]);
					}
				}
			}
		}
	...
	SmartVariantManagement.prototype._applyDefaultFavoritesForSelectionVariants = function(aVariantKeys) {
		...
		aVariantKeys.forEach(function(sVariantKey) {
			...
			if (oVariantItem) {
				var vFavorite = this._checkForAdaptationInfoFavorite(sVariantKey);

				if (vFavorite != undefined) {
					oVariantItem.setFavorite(vFavorite);
				} else {
	...
	SmartVariantManagement.prototype._checkForAdaptationInfoFavorite = function(sKey) {
		var oAdaptionInfo = this.getProperty("adaptationInfo");

		return (oAdaptionInfo && oAdaptionInfo.addFavorites) ? oAdaptionInfo.addFavorites[sKey] : undefined;
	};
	...
	SmartVariantManagement.prototype._applyDefaultFavorites = function(aVariantKeys) {
		...
		if (!this._sAppStandardVariantKey) {
			var vFavorite = this._checkForAdaptationInfoFavorite(this.STANDARDVARIANTKEY);
			if (vFavorite != undefined) {
				this.setStandardFavorite(vFavorite);
			} else {
...
//				} else
	...
	SmartVariantManagement.prototype._getDefaultVariantKey = function() {
		...
		if (!sDefaultVariantKey) {
			sDefaultVariantKey = this._getAdaptationInfoDefaultVariantKey();
		}

		return sDefaultVariantKey;
	};

	SmartVariantManagement.prototype._getAdaptationInfoDefaultVariantKey = function() {
		var sDefaultVariantKey = "";
		var oAdaptionInfo = this.getProperty("adaptationInfo");
		if (oAdaptionInfo && oAdaptionInfo.defaultVariant) {
			sDefaultVariantKey = oAdaptionInfo.defaultVariant;
		}
...
//			if (!oVariant.isUserDependent()) {
//				var oAdaptionInfo = this.getProperty("adaptationInfo");
//				if (oAdaptionInfo && oAdaptionInfo.executeOnSelect.hasOwnProperty(oVariant.getId())) {
//					return oAdaptionInfo.executeOnSelect[oVariant.getId()];
//				}
//			}
	...
	SmartVariantManagement.prototype._setExecuteOnSelections = function(aVariantInfo) {
		...
		if (aVariantInfo && aVariantInfo.length) {
			...
			for (i = 0; i < aVariantInfo.length; i++) {
				...
				if (oChange) {
					if (oChange.isReadOnly() && this.getSupportExecuteOnSelectOnSandardVariant() && (aVariantInfo[i].key === this.getStandardVariantKey())) {
						this._setExecuteOnSelectOnStandardVariant(aVariantInfo[i].exe);
						continue;
					}
	...
	SmartVariantManagement.prototype._addFavoriteToFlex = function(oFavorite) {
		FlexWriteAPI.add(oFavorite);
	};

	SmartVariantManagement.prototype._setFavorites = function() {
		if (this._aFavorites && (this._aFavorites.length > 0)) {
			this._aFavorites.forEach(function(oFavorite) {
				this._addFavoriteToFlex(oFavorite);
			}.bind(this));
	...
	SmartVariantManagement.prototype._save = function(bNewVariant, bIgnoreVariantHandling) {
		...
		if (FlexWriteAPI) {
			try {
				// create favorites for default at latest point in time
				this._setFavorites();
	...
	SmartVariantManagement.prototype._triggerGeneralSelectVariant = function(sVariantKey, sContext) {
		...
		if (oContent) {
			...
			// not yet in scope
//			var oItem = this.getItemByKey(sVariantKey);
//			if (oItem && oItem.getGlobal()) {
//				var oAdaptionInfo = this.getProperty("adaptationInfo");
//				if (oAdaptionInfo && oAdaptionInfo.executeOnSelect && oAdaptionInfo.executeOnSelect[sVariantKey]) {
//					oContent.executeOnSelection = oAdaptionInfo.executeOnSelect[sVariantKey];
//				}
//			}
