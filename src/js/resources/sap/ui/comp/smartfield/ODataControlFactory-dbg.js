...
sap.ui.define([
	...
	ODataControlFactory.prototype._createEdmString = function() {
		...
		if (this.oTextArrangementDelegate.bValidMetadata) {
			...
			if (this._oHelper.oAnnotation.isUpperCase(this._oMetaData.property.property)) {
				oSettings.sDisplayFormat = "UpperCase";
			}
	...
	ODataControlFactory.prototype._handleEventingForTextArrangement = function(oSettings) {
		...
		oControl.attachChange(function (oEvent) {
			try {
				this._oParent.fireChange(arguments);
			} catch (oException) {
				Log.error(oException);
			}
	...
	ODataControlFactory.prototype._createEdmUOM = function() {
		...
		oMeasureField.addAriaLabelledBy(oCurrencySmartField.getId() + "-text");
	...
	ODataControlFactory.prototype._checkTextInDisplayModeValueList = function () {
		...
		return !!(
			...
			(this._oMetaData.property.property.type === "Edm.String" || this._oMetaData.property.property.type === "Edm.Guid") &&
			...
			!this._oHelper.oAnnotation.getText(oEdmProperty) // There should be no text annotation assigned to the main property
		);
	};
	...
	ODataControlFactory.prototype.bind = function() {
		...
		.then(function onValueListAnnotationLoaded(oValueListAnnotations) {
			...
			if (oValueListAnnotations && this.oTextArrangementDelegate) {
				...
				this._oParent.attachModelContextChange(this._handleModelContextChangeEvent);
			}
		}.bind(this))
		.catch(function(oError) {
