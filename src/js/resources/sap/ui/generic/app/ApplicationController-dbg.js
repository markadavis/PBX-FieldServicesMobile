...
sap.ui.define([
	...
	ApplicationController.prototype._onValidateFieldGroup = function (aGroups, oView) {
		...
		for (i = 0; i < len; i++) {
			aValiditiyPromises.push(this._executeFieldGroup(aGroups[i], mRequests, oView));
		}
		var fnValidityPromisesCallback = function() {
		...
		return Promise.all(aValiditiyPromises).then(fnValidityPromisesCallback, fnValidityPromisesCallback);
	};
	...
	ApplicationController.prototype._executeSideEffects = function (oSideEffects, oContext, mParameters, mRequests) {
		var that = this, fFunction, sMethod, sTriggerAction, mMethods = {
		...
		if (that.getTransactionController().getDraftController().getDraftContext().hasDraft(oContext)) {
			...
			} else if (oSideEffects.EffectTypes && oSideEffects.EffectTypes.EnumMember) {
