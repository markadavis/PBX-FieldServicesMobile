...
sap.ui.define([
	...
	NodeContentType
		...
		ViewStateManager.prototype.onRemoveViewStateManager = function(viewStateManager) {
		...
		*/
		...
		Viewport.prototype._clearAnnotations = function() {
		...
		*/
		...
		Viewport.prototype._isPanoramicActivated = function() {
			var nativeCamera = this._getNativeCamera();
			var bgdImageType = this._viewStateManager && this._viewStateManager.getBackgroundImageType();
			return nativeCamera.userData.rotate && bgdImageType !== "2DImage";
		};
		...
		function updateWorldMatrixRecursive(node, force) {
			...
			for (var i = 0, l = children.length; i < l; i++) {
				updateWorldMatrixRecursive(children[i], force);
			}
		}
		...
		Viewport.prototype.render = function() {
			...
			updateWorldMatrixRecursive(nativeScene, false);
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				boundingBox = this._scene._computeBoundingBox(layers, false, true);
				if (vsm) {// expand the bounding box with selected objects because they can be invisible
					vsm._selectedNodes.forEach(function(node) {
						node._expandBoundingBox(boundingBox, null, false, false);
					});
				}
	...
	// remove selection for snapshot
		...
		Viewport.prototype.getObjectImage = function(nodeRef, width, height, topColor, bottomColor, quaternion, margin) {
			...
			nodeRef._expandBoundingBox(boundingBox, null, false, false);
		...
		Viewport.prototype._setContent = function(content) {
			...
			if (content) {
				...
				if (camera instanceof OrthographicCamera) {
					...
					boundingBox = scene._computeBoundingBox(this._getLayers(), true);
				...
				if (content.loaders) {
					for (i = 0; i < content.loaders.length; i++) {
						if (content.loaders[i] instanceof ContentDeliveryService) {
							...
							this._cdsLoader.attachLoadingFinished(function(){
								this._clearAnnotations();
								this.getAnnotations();
								this.setShouldRenderFrame();
							}, this);
		...
		*/
		...
		Viewport.prototype.setViewInfo = function(viewInfo, flyToDuration) {
			...
			if (selection && vsm) {
				...
				if (Array.isArray(selection.outlined)) {
					this._viewStateManager.setSelectionState();
					var outlined = [];
					var unoutlined = [];
					...
					if (view) {
						...
						for (var n = 0; n < viewAnnotationNodes.length; n++) {
							...
							if (node.target._vkPersistentId() === annotationInfo.node._vkPersistentId()) {
								// This annotation is in current view, create it
		 ...
		 * @param {object} options.parametricContent patametric content object
