...
sap.ui.define([
	...
	*/
	ViewStateManager.prototype.setVisibilityState = function(nodeRefs, visible, recursive) {
		// console.log("setVisibilityState", nodeRefs, visible, recursive);
	...
	/**
	 * Get visibilty state of the node taking its ancestors' visibility into account.
	...
	*/
	...
	ViewStateManager.prototype._getBackgroundNodeInfos = function() {
		if (this._currentBackgroundNodeInfos) {
			return this._currentBackgroundNodeInfos;
		}
		...
		var view = this.getCurrentView() || this._scene.getViews()[0];
		if (view) {
			var nodeRefs = view.getNodeInfos().reduce(function(nodes, curr) {
				if (curr.visible) { nodes.push(curr.target); }
				return nodes;
			}, []);
			while (nodeRefs.length) {
				var node = nodeRefs.shift();
				if (node._vkGetNodeContentType() === NodeContentType.Background
					&& node.parent	// node is still in the rendering tree
					&& nodeNameImageType[ node.name ]
					&& node.userData.treeNode
					&& this._getEffectiveVisibility(node) !== false) {
					currBackgroundNode = node;
					break;
				}
				// check the new added child first
				for (var i = node.children.length - 1; i >= 0; --i) {
					if (node.children[i].layers.test(this._layers)) {
						nodeRefs.push(node.children[i]);
					}
				}
			}
		...
		this._currentBackgroundNodeInfos = { current: currBackgroundNode, all: backgroundNodes };
		return this._currentBackgroundNodeInfos;
	};
	...
	ViewStateManager.prototype.getBackgroundImageType = function() {
		...
		return currBackgroundNode ? nodeNameImageType[ currBackgroundNode.name ] : null;
	};
	...
	ViewStateManager.prototype.setBackgroundImageType = function(imageType) {
		...
		this._currentBackgroundNodeInfos = null;
			...
			if (nodeInfo) {  // for totaraLoader
				...
				if (setVisibility) {
					...
					nodeInfo.forEach(function(info) {
						if (info.target.parent === null || info.target.layers !== info.target.parent.layers) {
	...
	ViewStateManager.prototype._prepareTransition = function(view) {
		...
		var collectTransitionMeshes = function(nodeRef, visible) {
			if (nodeRef.layers !== nodeRef.parent.layers) {
				visible = visibleNodes.has(nodeRef);
			}

			if (nodeRef.geometry) {
				if (visible) {
					if (!nodeRef.layers.test(layers)) {
						fadeInNodes.push(nodeRef);
					}
				} else if (nodeRef.layers.test(layers)) {
					fadeOutNodes.push(nodeRef);
				}
			}
			...
			nodeRef.children.forEach(function(child) {
				collectTransitionMeshes(child, visible);
			});
		}.bind(this);

		nodeHierarchy.getChildren()[ 0 ].children.forEach(collectTransitionMeshes, true);
