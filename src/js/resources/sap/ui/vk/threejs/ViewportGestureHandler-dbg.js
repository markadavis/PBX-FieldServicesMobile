			...
			var ThreeJsCameraController = function(vp) {
				...
				this.rotate = function(dx, dy, isTurnTableMode) {
					...
					if (viewport._isPanoramicActivated()) {
						dx *= -0.2;
						dy *= -0.2;
						// reset gesture target in sphere background
						gestureTarget.setScalar(0);
					}
				...
				this.zoom = function(zoomFactorScaler) {
					...
					if (!camera.userData.isRedlineActivated) {
						...
						var ignoreDynamicObjects = camera.userData.rotate ? false : true;  // for parametric image plane zoom out
						viewport.getScene().getSceneRef()._expandBoundingBox(boundingBox, viewport._getLayers(), ignoreDynamicObjects);
				...
				this.zoomBox = function(boundingBox, margin, timeInterval, nodeRef, isIn) {
					...
					this._transitionEffect = undefined;
