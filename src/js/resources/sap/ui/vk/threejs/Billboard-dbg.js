...
sap.ui.define([
	...
	Billboard.prototype._setBillboardTexture = function(canvas, width, height) {
		...
		this._billboard.geometry.setAttribute("uv", new THREE.Float32BufferAttribute([ 0, 1 - v, u, 1 - v, 0, 1, u, 1 ], 2));
	...
	Billboard.prototype._update = function(renderer, camera, viewportSize) {
		...
		if (coordinateSpace === BillboardCoordinateSpace.Screen) {// image note
			scale = THREE.Math.lerp(camera.near, camera.far, 1e-4); // as close as possible to the near clipping plane
	...
	Billboard.prototype._ortho2DUpdate = function(renderer, camera) {
		if (!this.layers.test(camera.layers)) {
			return;
		}
	...
	Billboard.prototype._lockToViewportUpdate = function(renderer, camera) {
		...
		this.children.forEach(function(child) {
			if (child.layers.test(camera.layers) && Math.abs(child.quaternion.w - 1) > 1e-3) { // take into account only rotated visible nodes
