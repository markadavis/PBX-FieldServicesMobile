...
sap.ui.define([
	...
	Annotation.prototype.setHeight = function(height) {
		...
		if (this._viewport && this._textDiv && this._textDiv.getDomRef()) {
			...
			if (this._textDiv.getNativeApi){
				if (this._resized) {
					textDivDom.style.height = "calc(" + normRect.height + "px)";
				} else {
					textDivDom.style.height = "calc(" + normRect.height + "px + 0.6rem)";
				}
			}else {
				textDivDom.style.height = "calc(" + normRect.height + "px - 1rem)";
			}
		}
	};
	...
	Annotation.prototype.setWidth = function(width) {
		...
		if (this._viewport && this._textDiv && this._textDiv.getDomRef()) {
			...
			if (this._textDiv.getNativeApi){
				if (this._resized) {
					textDivDom.style.width = "calc(" + normRect.width + "px)";
				} else {
					textDivDom.style.width = "calc(" + normRect.width + "px + 0.6rem)";
				}
			}else {
	...
	Annotation.prototype.update = function() {
		...
		if (this._isPositionRelative() && aNode) {
			...
			if (aNode) {
				...
				// config leaderline options for different style
				...
				setLeaderLineSVG(annotationNodeElement, annotation, annotation.childNodes[10], {
					...
					toHeight: leaderOptions.toHeight  // leaderline point to height percentage: 0 top, 0.5 middle, 1 bottom
				});
			} else {
				annotationNodeElement.style.visibility = "hidden";
