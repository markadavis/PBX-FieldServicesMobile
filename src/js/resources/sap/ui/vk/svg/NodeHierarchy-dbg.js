...
sap.ui.define([
	...
	NodeHierarchy.prototype.findNodesByName = function(query) {
		...
		if (query === undefined || query === null || query === "" || jQuery.isEmptyObject(query)) {
			...
			if (searchModeFunction === undefined) {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT8.summary), Messages.VIT8.code, "sap.ui.vk.svg.NodeHierarchy");
			} else if (!Array.isArray(query.value) && typeof query.value !== "string") {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT6.summary), Messages.VIT6.code, "sap.ui.vk.svg.NodeHierarchy");
			} else {
	...
	*/
	...
	NodeHierarchy.prototype.createNode = function(parentNode, name, insertBeforeNode, nodeContentType, content) {
		...
		jQuery.sap.assert(parentNode.children[parentNode.children.length - 1] === element, getResourceBundle().getText("NODEHIERARCHY_MSG_CREATENODEFAILED"));
		if (insertIndex >= 0) {
