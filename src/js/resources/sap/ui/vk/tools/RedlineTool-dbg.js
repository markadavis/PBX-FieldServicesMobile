...
sap.ui.define([
	...
	RedlineTool.prototype.init = function() {
		...
		this.setFootprint([ "sap.ui.vk.threejs.Viewport", "sap.ui.vk.dvl.Viewport", "sap.ui.vk.NativeViewport" ]);
	...
	RedlineTool.prototype.setActive = function(value, activeViewport, gizmoContainer) {
		...
		if (viewport) {
			if (value) {
				if (viewport instanceof vkLibrary.threejs.Viewport) {
					viewport._viewportGestureHandler._activateRedline();
				} else if (viewport instanceof vkLibrary.dvl.Viewport) {
				...
				if (viewport instanceof vkLibrary.dvl.Viewport) {
