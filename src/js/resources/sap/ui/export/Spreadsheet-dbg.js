...
sap.ui.define([
	...
	function(jQuery, library, Core, ExportDialog, EventProvider, Device, SpreadsheetExport, Log, ExportUtils) {
		 ...
		 *          <li><code>version</code> (string) - Application version that creates the XLSX document (default: "1.84.38")</li>
		 ...
		 * @public
		 */
		var Spreadsheet = EventProvider.extend(CLASS_NAME, {
			...
			constructor: function(mSettings) {
				...
				['count', 'dataSource', 'fileName', 'fileType', 'showProgress', 'workbook', 'worker'].forEach(function(sProperty) {
		...
		var createDataSourceFromBinding = function(oBinding) {
			...
			if (typeof oBinding.getDownloadUrl === 'function') {
				...
				oDataSource = {
					...
					headers: bV4ODataModel ?  oModel.getHttpHeaders() : oModel.getHeaders(),
		...
		function _createBuildPromise(oSpreadsheet, mParameters) {
			return new Promise(function(fnResolve, fnReject) {
				...
				function onmessage(oMessage) {
					...
					if (!isNaN(oMessage.progress)) {
						if (progressDialog) {
							progressDialog.updateStatus(oMessage.progress, oMessage.fetched, oMessage.total);
						}
						oSpreadsheet.onprogress(oMessage.progress);
					}
					...
					if (oMessage.finish) {
						...
						if (executeDefaultAction) {
							ExportUtils.saveAsFile(new Blob([oMessage.data], {
					...
					if (typeof oMessage.error != 'undefined') {
						...
						fnReject(oMessage.error);
						ExportDialog.showErrorMessage(oMessage.error);
					}
				}
